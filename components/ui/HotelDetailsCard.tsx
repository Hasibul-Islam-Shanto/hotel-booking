...
const HotelDetailsCard = ({ hotel }: { hotel: Hotel }) => {
  return (
    <Link href={`/hotels/${hotel?._id}`} className="block group">
      <div>
        ...
        <div className="mt-3">
          <div className="flex justify-between items-center">
            <h3 className="font-bold text-lg">{hotel?.propertyName}</h3>
            <div className="flex items-center">
              ...
              <span className="ml-1 text-zinc-600">{hotel?.averageRating}</span>
            </div>
          </div>
          <p className="text-zinc-500 text-sm mt-1">
            {hotel?.propertyLocation}
          </p>
